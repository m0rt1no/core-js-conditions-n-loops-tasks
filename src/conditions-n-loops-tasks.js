...
function convertToRomanNumerals(num) {
  ...
  if (num >= 10 && num < 20) {
    return roman[10] + roman[num - 10];
  }
  if (num >= 20 && num < 30) {
    return roman[20] + roman[num - 20];
  }
  if (num >= 30 && num < 40) {
    return roman[30] + roman[num - 30];
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < l; i += 1) {
    for (let j = 0; j < Math.floor(l / 2); j += 1) {
      const temp = resMatrix[i][j];
      resMatrix[i][j] = resMatrix[i][l - 1 - j];
      resMatrix[i][l - 1 - j] = temp;
    }
  }
  return resMatrix;
}
  ...
  for (let i = str.length - 2; i >= 0; i -= 1) {
    if (str[i] < str[i + 1]) {
      ...
      for (let j = i + 1; j < str.length; j += 1) {
        if (str[j] > str[i] && str[j] <= str[swap]) {
          swap = j;
        }
      }
      [str[i], str[swap]] = [str[swap], str[i]];

      const right = str.splice(i + 1).sort((a, b) => a - b);
      return Number([...str, ...right].join(''));
    }
  }
